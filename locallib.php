...
class assign {
    ...
    public function add_instance(stdClass $formdata, $callplugins) {
        ...
        // eduplag
        $formdata->checkplagiarism = !empty($formdata->checkplagiarism) ? 1 : 0;
        $update->checkplagiarism = $formdata->checkplagiarism;

        // Nếu checkplagiarism = 0 thì bắt buộc viewstudent = 0
        if (!$update->checkplagiarism) {
            $update->viewstudent = 0;
        } else {
            $update->viewstudent = !empty($formdata->viewstudent) ? 1 : 0;
        }
        $update->viewstudent = $formdata->viewstudent;
    ...
    public function add_all_plugin_settings(MoodleQuickForm $mform) {
        ...
        // eduplag
        $mform->addElement('advcheckbox', 'checkplagiarism', '', get_string('checkplagiarism', 'mod_assign'), null, [0, 1]);
        $mform->setDefault('checkplagiarism', 0);
        $mform->setType('checkplagiarism', PARAM_BOOL);
        $mform->addHelpButton('checkplagiarism', 'checkplagiarism', 'mod_assign');
        
        $mform->addElement('advcheckbox', 'viewstudent', '', get_string('viewstudent', 'mod_assign'), null, [0, 1]);
        $mform->setDefault('viewstudent', 0);
        $mform->setType('viewstudent', PARAM_BOOL);
        $mform->addHelpButton('viewstudent', 'viewstudent', 'mod_assign');
        
        // Ẩn viewstudent nếu chưa check checkplagiarism.
    ...
    public function remove_submission($userid) {
        ...
        // =======================
        $DB->execute("UPDATE {assignsubmission_file} SET plagiarism = '' WHERE submission = ?", [$submission->id]);   
