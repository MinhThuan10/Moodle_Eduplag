...
function xmldb_assign_upgrade($oldversion) {
    ...
    if ($oldversion < 2025041401) {
        $table = new xmldb_table('assign');
        $field = new xmldb_field('checkplagiarism', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, '0', 'timemodified');
    
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
            echo ">> Thêm field checkplagiarism vào bảng assign\n";
        } else {
            echo ">> Field checkplagiarism đã tồn tại\n";
        }
    ...
    if ($oldversion < 2025041402) {
        $table = new xmldb_table('assign');
        $field = new xmldb_field('viewstudent', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, '0', 'timemodified');
    
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
            echo ">> Thêm field viewstudent vào bảng assign\n";
        } else {
            echo ">> Field viewstudent đã tồn tại\n";
        }
    
        upgrade_mod_savepoint(true, 2025041402, 'assign');
    }
