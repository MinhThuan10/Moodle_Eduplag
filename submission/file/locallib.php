...
class assign_submission_file extends assign_submission_plugin {
    ...
    public function save_settings(stdClass $data) {
        // Eduplag
        // Nếu checkplagiarism được bật thì ép maxfilesubmissions = 1
        if (!empty($data->checkplagiarism)) {
    ...
    public function save(stdClass $submission, stdClass $data) {
        ...
        if (isset($instance->checkplagiarism) && (int)$instance->checkplagiarism === 1) {
            ...
            if (!empty($files)) {
                foreach ($files as $file) {
                    ...
                    $post_data = [
                        ...
                        'callback_url'     => $CFG->wwwroot . '/mod/assign/call_api_eduplag.php'
                    ];
        ...
        if ($filesubmission) {
            $filesubmission->numfiles = $this->count_files($submission->id,
                                                           ASSIGNSUBMISSION_FILE_FILEAREA);
            debugging('Submission File Data: ' . print_r($filesubmission, true), DEBUG_DEVELOPER);
    ...
    public function remove(stdClass $submission) {
        ...
        if ($currentsubmission) {
            ...
            $currentsubmission->plagiarism = "";
            $currentsubmission->urlfilechecked = "";
            ...
            $payload = json_encode([
                'school_name' => $fullname,
                'assignment_id' => $this->assignment->get_context()->id,
                'school_key'  => $schoolkey,
                'email'            => $USER->email,
                'class_id'         => $this->assignment->get_course()->id,
                'assignment_id'    => $this->assignment->get_instance()->id,
