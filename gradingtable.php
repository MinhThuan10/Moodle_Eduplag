...
class assign_grading_table extends table_sql implements renderable {
            ...
            foreach ($this->assignment->get_submission_plugins() as $plugin) {
                if ($this->is_downloading()) {
                    ...
                    if ($plugin->is_visible() && $plugin->is_enabled() && $plugin->has_user_summary()) {
                        ...
                        if ($instance->checkplagiarism == 1) {
                            $columns[] = 'plagiarism';
                            $headers[] = get_string('plagiarism', 'assignsubmission_file');

                            $columns[] = 'urlfilechecked';
                            $headers[] = get_string('urlfilechecked', 'assignsubmission_file');


                            $columns[] = 'checkedfile';
                            $headers[] = get_string('checkedfile', 'assignsubmission_file');
    ...
    public function col_plagiarism($row) {
        ...
        $data = $DB->get_record('assignsubmission_file', ['submission' => $row->submissionid]);
    ...
    public function col_urlfilechecked($row) {
        ...
        // Tạo thẻ <a> với văn bản ngắn gọn
    ...
    public function col_checkedfile($row) {
        ...
        // Icon PDF (bạn có thể đổi 'pdf' thành định dạng khác nếu cần)
        ...
        // Trả về thẻ a có icon và tên file
