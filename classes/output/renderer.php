...
class renderer extends \plugin_renderer_base {
    ...
    public function render_assign_submission_status_compact(\assign_submission_status_compact $status) {
        ...
        if ($submission) {
            ...
            global $DB;
            $asignment_data = $DB->get_record('assign', ['id' => $submission->assignment]);
            if ($asignment_data->checkplagiarism == 1) {

                $plagiarism_data = $DB->get_record('assignsubmission_file', ['submission' => $submission->id]);

                if ($plagiarism_data && !empty($plagiarism_data->plagiarism)) {
                    // Plagiarism Score
                    $o .= $this->output->container(
                        get_string('plagiarism', 'assignsubmission_file') . ': ' . $plagiarism_data->plagiarism . '%'
                    );
                
                    // Report URL (clickable link)
                    if (!empty($plagiarism_data->urlfilechecked)) {
                        $reporturl = \html_writer::link(
                            $plagiarism_data->urlfilechecked,
                            get_string('reportlink', 'assignsubmission_file'), // hoặc "Xem báo cáo"
                            ['target' => '_blank', 'rel' => 'noopener']
                        );
                        $o .= $this->output->container(
                            get_string('urlfilechecked', 'assignsubmission_file') . ': ' . $reporturl
                        );
                    }
                    
                
                    // Checked file download (PDF or other)
                    $fs = get_file_storage();
                    ...
                    if (!empty($files)) {
                        ...
                        $link = \html_writer::link(
                            $downloadurl,
                            \html_writer::tag('i', '', ['class' => 'icon fa fa-file-pdf-o', 'aria-hidden' => 'true']) . ' ' .
                            $file->get_filename(),
                            ['target' => '_blank']
                        );
                        $o .= $this->output->container(
                            get_string('checkedfile', 'assignsubmission_file') . ': ' . $link
                        );
                    }
                }
            }
    ...
    public function render_assign_submission_status(assign_submission_status $status) {
        ...
        if ($submission) {
            ...
            if (!$status->teamsubmission ||
                ($status->teamsubmission && ($status->submissiongroup || !$status->preventsubmissionnotingroup))) {
                ...
                foreach ($status->submissionplugins as $plugin) {
                    ...
                    if ($plugin->is_enabled() &&
                        ...
                        // Eduplag
                        global $DB;
                        $plagiarismdata = $DB->get_record('assignsubmission_file', ['submission' => $submission->id]);
                        $asignment_data = $DB->get_record('assign', ['id' => $submission->assignment]);
                        if ($plugin->get_type() === 'file' && $submission && $asignment_data->viewstudent == 1) {
                            
                            if ($plagiarismdata) {
                                // Score.
                                $cell1content = get_string('plagiarism', 'assignsubmission_file');
                                $cell2content = $plagiarismdata->plagiarism . '%';
                                $this->add_table_row_tuple($t, $cell1content, $cell2content);

                                // URL báo cáo.
                                if (!empty($plagiarismdata->urlfilechecked)) {
                                    $cell1content = get_string('urlfilechecked', 'assignsubmission_file');
                                    $url = \html_writer::link($plagiarismdata->urlfilechecked, 'View Report', ['target' => '_blank']);
                                    $this->add_table_row_tuple($t, $cell1content, $url);
                                }

                                // File đã kiểm tra đạo văn.
                                $fs = get_file_storage();
                                $context = \context_module::instance($status->coursemoduleid);
                                $files = $fs->get_area_files(
                                    $context->id,
                                    'assignsubmission_file',
                                    'checked_file',
                                ...
                                if (!empty($files)) {
                                    ...
                                    $cell1content = get_string('checkedfile', 'assignsubmission_file');
                                    $this->add_table_row_tuple($t, $cell1content, $link);
                                }
                            }
                        }
                    }
